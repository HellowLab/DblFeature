...
declare interface API {
  /**
   * Programmatically trigger a swipe of the card in one of the valid directions `'left'`, `'right'`, `'up'` and `'down'`. This function, `swipe`, can be called on a reference of the Swiper instance. Check the [example](https://github.com/3DJakob/react-tinder-card-demo/blob/master/src/examples/Advanced.js) code for more details on how to use this.
   *
...
declare interface Props {
  ...
  /**
   * Callback that will be executed when a `Swiper` has left the screen. It will be called with a single string denoting which direction the swipe was in: `'left'`, `'right'`, `'up'` or `'down'`.
   */
  ...
  /**
   * Callback that will be executed when a `Swiper` has unfulfilled the requirement necessary to be swiped in a direction on release.
   */
...
declare const Swiper: React.FC<Props>;
